...
Fresh start, deliberately basic: one note per project, one note per task, and a
`Home.md` dashboard that rolls them up. No build step, no server — open the
folder in Obsidian and it works. Same markdown format Holocron is specced to
use, so the two can share one vault later if you want.
...
- New project / new task: `Ctrl/Cmd+P` → "Templates: Insert template" → pick
